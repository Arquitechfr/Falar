...
export async function sendOtpController(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const { isNewUser, keySalt } = await sendOTP(phone);
    res.json({ success: true, isNewUser, keySalt });
  } catch (err) {
...
export async function verifyOtpController(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { phone, code, publicKey, deviceToken, keySalt } = verifyOtpSchema.parse(req.body);
    const result = await verifyOTPAndLogin(phone, code, publicKey, deviceToken, keySalt);
    res.json(result);
  } catch (err) {
    if (err instanceof AuthError) {
      res.status(err.statusCode).json({ error: { code: err.code, message: err.message } });
